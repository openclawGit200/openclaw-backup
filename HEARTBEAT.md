...
- [x] **PDF OCR**：南港 PDF 第9-11頁為圖片掃描，pypdf 讀不到，需整合 tesseract OCR（待處理）
    [x] **CF Dashboard**：✅ v3 完成（統一記憶：notes+twse+memory+obsidian）；Research tab 移除；URL: `https://3a38c74f.openclaw-dashboard-dv8.pages.dev/`；Repo: `openclawGit200/openclaw-dashboard`；⚠️ 自訂網域仍指向舊預設頁
